...
import { collection, getDocs, query, orderBy, where, deleteDoc, doc, getDoc, updateDoc } from 'firebase/firestore';
...
const TaxedInvoicesPage = () => {
  ...
  const handleDeleteInvoice = async () => {
    ...
    try {
      ...
      } else if (invoiceToDelete.source === 'taxedInvoices' || invoiceToDelete.source === 'taxed-invoices') {
        collectionsToCheck.push('taxedInvoices');
      } else if (invoiceToDelete.orderType === 'corporate' || invoiceToDelete.source === 'corporate-orders') {
      ...
      // Update local state
      setTaxedInvoices(prev => prev.filter(invoice => invoice.id !== invoiceToDelete.id));
          ...
          <Box sx={{ 
            ...
            <Typography variant="body2" sx={{ color: '#cccccc', mb: 1 }}>
              <strong>Customer:</strong> {invoiceToDelete?.orderType === 'corporate' 
                ...
                : (invoiceToDelete?.customerInfo?.customerName || invoiceToDelete?.originalCustomerInfo?.customerName || 'N/A')}
            </Typography>
